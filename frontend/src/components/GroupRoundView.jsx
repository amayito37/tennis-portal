...
/**
 * Presentational component that renders the same "group page" UI
 * for any { group, players, standings, fixtures, matches } dataset.
 *
 * If you pass `onReportMatch`, it will show the "Añadir resultado" button
 * when `canReportFixture(fixture)` is true.
 */
export default function GroupRoundView({
  group,
  players = [],
  standings = [],
  fixtures = [],
  ...
  // Reporting
  onReportMatch = null,
  canReportFixture = null,
}) {
  const defaultCanReport = (f) =>
    me && (f.player1.id === me.id || f.player2.id === me.id || me.is_admin);

  const _canReport = canReportFixture || defaultCanReport;
  const showActions = typeof onReportMatch === "function";
